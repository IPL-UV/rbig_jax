...
parser.add_argument("--wandb-project", type=str, default="gf_mnist_ms")
# =====================
# Testing
# =====================
parser.add_argument(
...
if wandb_logger.config.dataset == "mnist":
    ...
    train_ds = load_dataset("train", wandb_logger.config.batch_size)
    init_ds = load_dataset("train", wandb_logger.config.n_init_samples)
    valid_ds = load_dataset("test", wandb_logger.config.val_batch_size)
else:
    raise ValueError(f"Unrecognized dataset: {wandb_logger.config.dataset}")
...
if wandb_logger.config.model == "rqsplines":
    ...
    # init model
    X_g, gf_model = init_multiscale_flow_spline(
        seed=42,
    ...
    from rbig_jax.models.multiscale import init_multiscale_flow

    # init model
    X_g, gf_model = init_multiscale_flow(
        seed=42,
        image_shape=image_shape,
        filters=filters,
        X=X_init,
...
try:
    fig, ax = plot_image_grid(X_g, image_shape)
...
X_demo_approx = gf_model.inverse(X_g[:50])
...
optimizer = init_optimizer(
    ...
    cosine_decay_steps=wandb_logger.config.cosine_decay_steps,
    warmup=None,
    gradient_clip=wandb_logger.config.gradient_clip,
    alpha=wandb_logger.config.lr_alpha,
)
...
with tqdm.trange(wandb_logger.config.epochs) as pbar:
    for step in pbar:
        ...
        if step % wandb_logger.config.eval_freq == 0:
            joblib.dump(nf_trainer.model, model_save_name)
